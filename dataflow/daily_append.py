...
if __name__ == "__main__":
    ...
    filename = f"raw_dataflow/daily{date_str}"
    file_path = f'gs://earthquake-project-main-bucket/{filename}'
    read_path = f'{file_path}.json'
    ...
    date_str = datetime.datetime.now().strftime('%Y%m%d')
    filenames = f"silver_dataflow/daily{date_str}"
    ...
    with beam.Pipeline(options=options) as p:
        ...
        res = (
                p
                | 'Create PCollection' >> beam.io.ReadFromText(read_path)
